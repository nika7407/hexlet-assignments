...
public class PostsController {
    ...
    public static void change(Context ctx) {
        try {
            ...
            var redactedPost = new Post(post.getId(), name, body);
            PostRepository.save(redactedPost); // Исправлено: сохраняем обновлённый пост
            ctx.redirect(NamedRoutes.postsPath());
            ...
            var name = ctx.formParam("name");
            var body = ctx.formParam("body");
            var page = new EditPostPage(id, name, body, e.getErrors()); // Передаём id
            ctx.render("posts/edit.jte", model("page", page)).status(422); // Рендерим edit.jte
        }
    }
